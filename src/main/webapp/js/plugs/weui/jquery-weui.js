...
+ function($) {
  ...
  $.prompt = function(text, title, onOK, onCancel, input,maxlength) {
    var config;
    if (typeof text === 'object') {
      config = text;
    } else {
    ...
    var gerr="";
    if(config.maxlength){
    	gerr='maxlength="'+config.maxlength+'"'
    }else{
    	gerr="";
    }
    
    var modal = $.modal({
      text: '<p class="weui-prompt-text">'+(config.text || '')+'</p><input type="text" class="weui_input weui-prompt-input"  '+gerr+' id="weui-prompt-input" value="' + (config.input || '') + '" />',
      title: config.title,
      autoClose: false,
      buttons: [
      {
