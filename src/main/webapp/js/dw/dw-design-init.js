...
//						quLogicItem.find("input[name='logicEvent']").val(logicEvent);
	...
	$("#dwDialogQuSetSave").click(function(){
		if(dwDialogObj!=null){
			...
			var setOptionOneTime=$("#dwCommonDialog input[name='setOptionOneTime']:checked");
			console.log(setOptionOneTime[0]);
			...
			var quType=quItemBody.find("input[name='quType']").val();
			if(quType=="RADIO"){				
				quItemBody.find("input[name='optionOneTime']").val(setOptionOneTime[0]?1:0);
				console.log(quItemBody.find("input[name='optionOneTime']")[0]);
			}			
	...
	*/
	...
	$("#publishBtn").click(function(){
		//检查是否有题
		var liSurveyQuItemBodys = $("#dwSurveyQuContentAppUl .li_surveyQuItemBody");
		if(!liSurveyQuItemBodys[0]){
			alert("至少保证有一题，才能发布！");
			return false;
		}
...
function showDialog(thDialogObj){
	...
	if(thObjClass.indexOf("addMoreOption")>=0){
		...
		var optionOneTime=quItemBody.find("input[name='optionOneTime']").val();
		console.log("aa"+optionOneTime);
		...
		$("#dwCommonDialog input[name='setOptionOneTime']").prop("checked",false);
		...
		$("#dwCommonDialog input[name='setOptionOneTime']").prop("checked",false);
		$("#dwCommonDialog .optionOneTime").hide();
		...
		//单选情况下才启用各选项仅可提交一次设置项
		if(quType=="RADIO"){			
			$("#dwCommonDialog .optionOneTime").show();
			if( optionOneTime==1){
				$("#dwCommonDialog input[name='setOptionOneTime']").prop("checked",true);
				$("#dwCommonDialog .optionOneTime").show();				
			}
		}
...
function saveRadio(quItemBody,callback){
	...
	if(saveTag==0){
		...
		var contactsField=quItemBody.find("input[name='contactsField']").val();
		var optionOneTime=quItemBody.find("input[name='optionOneTime']").val();
		console.log("optionOneTime:"+optionOneTime);
		...
		data+="&contactsAttr="+contactsAttr+"&contactsField="+contactsField+"&optionOneTime="+optionOneTime;
...
//				var optionText=$(this).prev().text();
				...
				$.each(logicQuOptionSels,function(){
					var selOptionVal=$(this).val();
					console.log("....selOptionVal........"+selOptionVal);
					console.log("....optionId........"+optionId);
					if(selOptionVal==optionId){
						console.log("....selOptionVal = optionId........");
						eachTag=false;
						return false;
					}
				});
				//alert(optionText);
			//	if(eachTag){   //xizhenyin
