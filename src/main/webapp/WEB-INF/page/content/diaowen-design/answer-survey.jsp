...
<script type="text/javascript" src="${ctx }/js/plugs/layer/layer.js"></script>
...
<%-- <script type="text/javascript" src="${ctx }/js/plugs/jbox-v2.3/jBox/jquery.jBox-2.3.min.js"></script>

<script type="text/javascript" src="${ctx }/js/plugs/jbox-v2.3/jBox/i18n/jquery.jBox-zh-CN.js"></script>
<script type="text/javascript" src="${ctx }/js/plugs/jbox-v2.3/jBox/jquery-migrate-1.1.1.js"></script>
<script type="text/javascript" src="${ctx }/js/plugs/jbox-v2.3/jBox/jquery-migrate-1.1.1.min.js"></script>

  <link type="text/css" rel="stylesheet" href="${ctx }/js/plugs/jbox-v2.3/jBox/Skins/Red/jbox.css"/> --%>
...
$(document).ready(function(){
	...
	$(".submitSurvey").click(function(){
		if(validateForms()){
/* 			top.$.jBox.confirm("提交后不能修改!是否提交问卷？","系统提示",function(v,h,f){
				if(v=="ok"){
					$("#surveyForm").submit();
				}
			}) */
...
//		if(validateForms()){
			/*if($("input[name='jcaptchaInput']:visible")[0]){
				var url="${ctx}/response!validateCheckcode.action";
				var data="jcaptchaInput="+$("input[name='jcaptchaInput']").val();
				$.ajax({
					url:url,
					data:data,
					type:"post",
					success:function(msg){
						if(msg==="succ"){
							...
							refreshAutoCode('register-jcaptchaImg');
							alert("验证码错误！");
			...
			//xizhenyin 添加提交确认xinx
	//		if(confirm("请认真核对后提交，每个单位只能提交一次，提交后不能修改!")){
	//			$("#surveyForm").submit();
	//		}
...
/* 				var data="stridlist="+stridlist+"&strvallist="+strvallist;
				var url="${ctx}/response!quChenScoresCheck.action";
				$.ajax({
					url:url,
					data:data,
					type:"post",
	...
	function checkOptionOneTime(radioObj,quItemId){		
		var url="${ctx}/response!ajaxCheckOption.action";
		var data="quItemId="+quItemId;
		$.ajax({
			url:url,
			data:data,
			type:"post",
			success:function(msg){				
				if(msg==1){
					var optionName = radioObj.next().text();
					console.log("radioObj"+radioObj);
					console.log("radioObjNext"+radioObj.next());
					console.log("optionName:"+optionName);
					layer.alert(optionName+"已提交答卷，不可重复作答",{icon:0});
					radioObj.prop("checked",false);
					var quItemBody=radioObj.parents(".li_surveyQuItemBody");
	...
	$(".dwQuOptionItemContent").click(function(){
		...
		var quType=quItemBody.find(".quType").val();		
		var optionOneTime=quItemBody.find(".optionOneTime").val();
	...
	if(optionOneTime==1){
				var quItemId=thObj.find("input[type='radio']:checked").val();
				var radioObj=thObj.find("input[type='radio']:checked");				
				checkOptionOneTime(radioObj,quItemId);				
			}else{
	...
	function foreachLg(){
		...
		$.each(quLogics,function(){
			...
			if(logicType==="2"){
				//逻辑类型为“显示”2  则初始化为隐藏
				if(loginItem.parents(".li_surveyQuItemBody").is(":hidden")){
					var hidQuItemBody=$(".quId[value='"+skQuId+"']").parents(".li_surveyQuItemBody");
					hidQuItemBody.hide();
					hidQuItemBody.addClass("hidFor"+logicId);
					hidQuItemBody.find(".answerTag").attr("disabled",true);
    ...
    function refShow(quItemBody){
		var quType=quItemBody.find(".quType").val();
		if("RADIO"===quType){
			//单选题
			var checkeds = quItemBody.find("input[type='radio']:checked");
	...
	//	$("#resultProgressRoot .progress-label").text("完成度："+newValue+"%");   xizhenyin
					...
					<li class="li_surveyQuItemBody surveyQu_${pageNo }"  style="${pageNo gt 1 ?'display: none':''}">
						<div class="surveyQuItemBody">
							...
							<div class="quInputCase" style="display: none;">
								...
								<input type="hidden" class="optionOneTime" value="${en.optionOneTime }" id="optionOneTime">
								<input type="hidden" class="answerTag" value="0" >
										...
										<c:when test="${en.hv eq 1 }">
											<ul class="transverse">
												...
												<li class="quCoItemUlLi">
													<div class="dwQuOptionItemContent">
													<label class="dwRedioStyle dwQuInputLabel" ></label>
													<input type="hidden" id="quId" value="${en.id }">
													<input type="radio" name="qu_${en.quType }_${en.id }" value="${item.id }" id="optionId"><label class="editAble quCoOptionEdit quCoOptionPadding">${item.optionName }</label>
										...
										<table class="quCoChenTable" >
												...
												<tr class="dwQuCoChenRowTr"><td class="quChenRowTd"><label class="editAble quCoOptionEdit">${rowItem.optionName }</label>
													<input type="hidden" name="item_qu_${en.quType }_${en.id }_${rowItem.id }"  value="cs_item_qu_${en.quType }_${en.id }_${rowItem.id }_" />
														...
														<c:forEach begin="1" end="10" var="scoreNum" >
					...
					<div class="surveyQuItemBody">
							<div class="surveyQuItem">
								...
								<div class="surveyQuItemContent" style="padding-top: 12px;height: 30px;min-height: 30px;">
									...
									<a href="#" id="exitSurvey" class="sbtn24 sbtn24_0" style="background: grey;" >退&nbsp;出</a>&nbsp;&nbsp;
								</div>
							</div>
					</div>
				</li>
...
<!-- Diaowen.net Button BEGIN -->
	<div id="webSiteFixedRight" class="websiteFixed" style="position: fixed;right: 0px;top: 20px;z-index: 9999;">
		<a id="mobileTdId" href="＃" style="width: 15px;background: #8CBCD1;background: #f7423d;display: block;padding: 5px;padding-top: 10px;padding-bottom:10px;font-weight: bold;color: white;cursor: pointer;float: right;vertical-align: middle;text-decoration: none;font-size: 12px;">手机地址</a>
