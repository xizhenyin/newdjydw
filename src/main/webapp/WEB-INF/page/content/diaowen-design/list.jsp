...
<body>
	...
	<div id="dwBody" style="margin-top:0px;padding-top: 0px;">
		<div id="dwBodyContent" class="" style="">
		<div id="dwBodyUser">
			<div class="surveyCollectMiddle">
				<div class="surveyCollectMiddleContent">
					<div style="padding: 25px 45px;overflow: auto;padding-top: 20px;">
							...
							所有问卷&nbsp;&nbsp;|&nbsp;&nbsp;
								<button href="${ctx }/design/my-survey-create!save.action?djyToken=${djyToken}" id="surveyAdd-a" class="sbtn25 sbtn25_1" style="outline: none;text-decoration: none;" target="_blank" ><i class="fa fa-plus " aria-hidden="true"></i>&nbsp;新建问卷</button>
							</div>

							<form action="${ctx}/design/my-survey.action?djyToken=${djyToken}" method="post" >
							...
							<div style="padding: 5px;color:#666565; ">
								<table class="contacts-table" width="100%" cellpadding="0" cellspacing="0">
									...
									<tr>
										...
										<td align="left"><a target="_blank" href="${ctx }/wenjuan/${en.sid }.html?djyToken=${djyToken}" class="titleTag">${en.surveyName }</a></td>
										...
										<td align="left">
											<div class="btn-group surveyLeftBtnGroup">
											  <a class="btn btn-default" href="${ctx }/design/my-survey-design.action?surveyId=${en.id}&djyToken=${djyToken}" title="设计"data-toggle="tooltip" data-placement="top" target="_blank" ><i class="fa fa-pencil-square-o"></i></a>
											  <a class="btn btn-default" href="${ctx }/design/my-collect.action?surveyId=${en.id}&djyToken=${djyToken}" title="收集答卷" data-toggle="tooltip" data-placement="top" target="_blank" ><i class="fa fa-comments-o"></i></a>
											  <a class="btn btn-default" href="${ctx }/da/survey-report!defaultReport.action?surveyId=${en.id}&djyToken=${djyToken}" title="分析报告" data-toggle="tooltip" data-placement="top" target="_blank" ><i class="fa fa-line-chart"></i></a>
											  ...
											  <a class="btn btn-default deleteSurvey" href="${ctx}/design/my-survey!delete.action?id=${en.id}&djyToken=${djyToken}" title="删除问卷" data-toggle="tooltip" data-placement="top" ><i class="fa fa-trash-o fa-fw"></i></a>
											</div>&nbsp;
									...
									<c:otherwise>
										<tr>
											<td colspan="7">
												...
												<div style="padding: 60px;font-size: 22px;text-align: center;color: #b1aeae;">暂无信息！</div>
								...
								<div style="padding-top: 15px;text-align: center;">
									<div class="btn-group">
										...
										<c:if test="${page.startpage > 1}">
											<a href="${ctx }/design/my-survey.action?page.pageNo=1&djyToken=${djyToken}" class="btn btn-default">1</a>
										...
										<c:forEach begin="${page.startpage }" end="${page.endpage }" var="en">
											<c:choose>
												<c:when test="${page.pageNo eq en }"><a href="${ctx }/design/my-survey.action?page.pageNo=${en }&djyToken=${djyToken}" class="btn btn-default" style="background: #efbfbe;">${en }</a></c:when>
												<c:otherwise><a href="${ctx }/design/my-survey.action?page.pageNo=${en}&djyToken=${djyToken}" class="btn btn-default">${en }</a></c:otherwise>
											</c:choose>
										</c:forEach>
										<c:if test="${page.totalPage > (page.endpage)}">
											...
											<a href="${ctx }/design/my-survey.action?page.pageNo=${page.totalPage}&djyToken=${djyToken}" class="btn btn-default">${page.totalPage }</a>
										</c:if>
										<c:if test="${page.totalPage > page.pageNo}">
											<a href="${ctx }/design/my-survey.action?page.pageNo=${page.pageNo+1}&djyToken=${djyToken}" class="btn btn-default">&gt;</a>
										</c:if>
...
//delete
		...
		$(".copySurvey").click(function(){
			...
			$("body").append("<div id=\"myDialogRoot\"><div class='dialogMessage' style='padding-top:40px;margin-left:20px;padding-bottom:0px;'>"+
					"<div>复制标题：<input id='surTitleTemp' type='text' style='padding:3px;width:320px;color:#e7726e;' value=''></div></div></div>");
				...
				buttons: {
					"OK":{
						...
						click: function() {
							...
							window.open("${ctx}/design/my-survey-design!copySurvey.action?"+params+"&djyToken=${djyToken}","_blank");
		...
		$("#surveyAdd-a").click(function(){
			...
			$("body").append("<div id=\"myDialogRoot\"><div class='dialogMessage' style='padding-top:40px;margin-left:20px;padding-bottom:0px;'>"+
					"<div>问卷标题：<input id='surTitleTemp' type='text' style='padding:5px;width:320px;color:#e7726e;' value=''></div></div></div>");
				...
				buttons: {
					"OK":{
						...
						click: function() {
							...
							window.open("${ctx}/design/my-survey-create!save.action?"+params+"&djyToken=${djyToken}","_blank");
							setTimeout(function(){
								window.location.reload();
							},1000);
						}
