...
//		0网调  1录入数据 2移动数据 3导入数据
	...
	public Long userAnswerCount(String userId,String beginDate, String endDate) {
		...
		String sqlExt = "";
		if(beginDate!=null){
			sqlExt+= " and end_an_date >= :beginDate ";
		}
		if(endDate!=null){
			sqlExt+= " and end_an_date <= :endDate ";
		}
		String sql = "select count(*) from t_survey_answer where user_id = :userId "+sqlExt;
	...
	public List<Object[]> groupUser(String beginDate, String endDate) {
		String sql = "select t1.login_name,t1.name,count(*) from t_user t1,t_survey_answer t2 where t1.id = t2.user_id and t2.end_an_date >= :beginDate and t2.end_an_date < :endDate group by t2.user_id";
		SQLQuery sqlQuery = this.getSession().createSQLQuery(sql);
		sqlQuery.setParameter("beginDate",beginDate);
		sqlQuery.setParameter("endDate",endDate);
	...
	@Override
	public List<String> userAnsered(String userId,String qId) {
		
		List<String> list = null;
		String sql = "select t1.id from t_survey_directory  t1 left JOIN t_survey_answer t2 on t2.survey_id = t1.id where t2.user_id = ?";
		if(qId != null) {
			sql += "and t1.id= ?";
		}
...
//		}
		if(qId == null) {
			 list =(List<String>)this.getSession().createSQLQuery(sql).setString(0, userId).list();
		}else {
			SQLQuery query=this.getSession().createSQLQuery(sql);
			query.setString(0, userId);
			query.setString(1, qId);
