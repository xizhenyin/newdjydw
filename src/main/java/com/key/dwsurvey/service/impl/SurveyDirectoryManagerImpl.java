...
public class SurveyDirectoryManagerImpl extends BaseServiceImpl<SurveyDirectory, String> implements SurveyDirectoryManager {
	...
	public Page<SurveyDirectory> findPage(Page<SurveyDirectory> page,
										  SurveyDirectory entity,String surveyState) {
		page.setOrderBy("createDate");
		page.setOrderDir("desc");
		//page.setPageSize(25);
		List<Criterion> criterions=new ArrayList<Criterion>();
		criterions.add(Restrictions.eq("visibility", 1));
		if(surveyState!=null && !"".equals(surveyState) && !"3".equals(surveyState)){
			criterions.add(Restrictions.eq("surveyState", Integer.parseInt(surveyState)));
		}
		criterions.add(Restrictions.isNull("isShow"));
		criterions.add(Restrictions.eq("dirType", 2));
		criterions.add(Restrictions.eq("surveyModel", 1));
		Integer share = entity.getIsShare();
		if(share!=null && share==1){
...
//		List<String> idList = surveyAnswerDao.userAnsered(accountManager.getCurUser().getId(),null);
//		if(idList != null && idList.size()>0) {
//			String arr = idList.get(0);
//			for (String objects : idList) {
//				map.put(objects,objects);
//			}
//		}
//	
//		
//		List<SurveyDirectory> newList = new ArrayList<SurveyDirectory>();
//		page = surveyDirectoryDao.findPageList(page, criterions);
//		for(SurveyDirectory sur : page.getResult()) {
//			if(null != map.get(sur.getId())) {
//				sur.setAnswerStatus("1");
//				newList.add(sur);
//			}
//		}
//		
//		SurveyDirectory surd = null;
//		List<SurveyDirectory> listt = page.getResult();
//		//surveyDirectoryDao.get(map.get(sur.getId()));
//		 for (Map.Entry<String, String> entry : map.entrySet()) {
//			   System.out.println("key= " + entry.getKey() + " and value= " + entry.getValue());
//			   
//			   surd = surveyDirectoryDao.get(entry.getKey());
//			   listt.remove(surd);
//			   
//			  }
//			page.setResult(listt);
//			page.setTotalItems(listt.size());
//		if("3".equals(surveyState)) {
//			page.setResult(newList);
//			page.setTotalItems(newList.size());
//		}
...
//		directory.setSurveyDetailId(detail.getId());
	...
	@Override
	public Long userSurveyCount(String id, String beginDate, String endDate) {
		return surveyDirectoryDao.userSurveyCount(id,beginDate,endDate);
	}

	
	public List<Object[]> groupUser(String beginDate,String endDate)  {
		return surveyDirectoryDao.groupUser(beginDate,endDate);
	}

	//判断是否答过题 xizhenyin
	@Override
	public List<String> currentUserAnsered(String qId) {
		// TODO Auto-generated method stub
		return surveyAnswerDao.userAnsered(accountManager.getCurUser().getId(),qId);
	}
}
