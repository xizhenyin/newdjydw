...
public class SurveyAnswerManagerImpl extends
	...
	@Override
	public List<AnRadio> findAnswerList(String quItemId){		
		Criterion criterionOptionId = Restrictions.eq("quItemId", quItemId);		
		return anRadioDao.find(criterionOptionId);
	}
	...
	private int getquestionAnswer(String surveyAnswerId, Question question) {
		...
		} else if (quType == QuType.CHENSCORE) {// 矩阵多选题
			List<AnChenScore> anChenScores = anChenScoreManager
					.findAnswer(surveyAnswerId, quId);
					...
					for (QuChenColumn quChenColumn : quChenColumns) {
						...
						for (AnChenScore anChenScore : anChenScores) {
							...
							//	answerOptionName=quChenColumn.getOptionName();
								...
								answerOptionName=anChenScore.getAnswserScore();
								break;
							}
						}
						answerOptionName=HtmlUtil.removeTagFromText(answerOptionName);
...
//			String quName = question.getQuName();
	...
	public Long userAnswerCount(String userId, String beginDate, String endDate) {
		return surveyAnswerDao.userAnswerCount(userId, beginDate, endDate);
	}

	public List<Object[]> groupUser(String bDate,String eDate){
		return surveyAnswerDao.groupUser(bDate,eDate);
	}
	@Override
	@Transactional
