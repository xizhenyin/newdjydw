...
@AllowedMethods({"previewDev","devSurvey","ajaxSave","copySurvey","buildSurveyForZbsh"})
public class MySurveyDesignAction extends ActionSupport{
	...
	public String devSurvey() throws Exception {
		...
		try{
			...
			new JspToHtml().postJspToHtml(url, filePath, fileName);
//			 if(surveyDirectoryManager.get(survey.getId())!=null) {
//				 reqUrl = ZbshConstants.UPDATEQUESTIONNAIRE_URL;
//			 }else{
//				 reqUrl =  ZbshConstants.CREATEQUESTIONNAIRE_URL;
//			 };
			survey.setSurveyState(1);
			surveyDirectoryManager.save(survey);   //TODO
			//xizhenyin 支部生活
			
	//		zbshJson(survey);  TODO
			//新增时问卷方需对调查问卷进行保存，同时将保存的相关信息以json形式返回支部生活，即调用支部生活的接口。 xizhenyin
...
//			JSONObject jsonObject=JSONObject.fromObject(survey,cfg);
//			SurveyDirectory sd = (SurveyDirectory) JSONObject.toBean(jsonObject, SurveyDirectory.class);
//			   entityToStr = gson.toJson(sd);
//			result.setErrorCode("200");
//			result.setMessage("正确");
//			result.setSuccess(true);
		
			//result.setData(jsonObject); //json
	...
	private void buildSurvey() {
		...
		if(user!=null){
		    ...
		    surveyDirectory=surveyDirectoryManager.getSurveyByUser(surveyId, userId);
...
//				SurveyDirectory survey=surveyDirectoryManager.getSurvey(surveyId);
	...
	public void buildSurveyForZbsh() {
		HttpServletRequest request=Struts2Utils.getRequest();
		HttpServletResponse response=Struts2Utils.getResponse();
		String svyName=request.getParameter("surveyName");
